export const dummydata = [
  {
    Image: require("../assets/Profile.png"),
    Name: "Andriy Pelyak",
    Text: "Good Morning 👋",
    Img: require("../assets/BMW.png"),
  },
];

export const Offers = [
  {
    Image: require("../assets/14.png"),
    percent: "20%",
    deals: "Week Deals",
    dics: "Get a new car discount, only valid this week",
  },
  {
    Image: require("../assets/15.png"),
    percent: "25%",
    deals: "Week Deals",
    dics: "Get a new car discount, only valid this week",
  },
  {
    Image: require("../assets/16.png"),
    percent: "22%",
    deals: "Week Deals",
    dics: "Get a new car discount, only valid this week",
  },
  {
    Image: require("../assets/17.png"),
    percent: "18%",
    deals: "Week Deals",
    dics: "Get a new car discount, only valid this week",
  },
];

export const sponsors = [
  {
    Title: "BMW",
    Img: require("../assets/BMW.png"),
  },
];
export const featured = [
  {
    Title: "BMW M4 Series",
    Image: require("../assets/13.png"),
    ratting: "4.5",
    disprice: "AED 115,000",
    price: "AED 175,000",
    new: "New",
  },
  {
    Title: "Camaro Sports",
    Image: require("../assets/14.png"),
    ratting: "4.7",
    disprice: "AED 175,000",
    price: "AED 175,000",
    new: "New",
  },
  {
    Title: "Audi Sports",
    Image: require("../assets/15.png"),
    ratting: "4.5",
    disprice: "AED 115,000",
    price: "AED 175,000",
    new: "New",
  },
  {
    Title: "McLaren Supercar",
    Image: require("../assets/16.png"),
    ratting: "4.9",
    disprice: "AED 115,000",
    price: "AED 175,000",
    new: "New",
  },
  {
    Title: "McLaren Supercar",
    Image: require("../assets/17.png"),
    ratting: "4.9",
    disprice: "AED 115,000",
    price: "AED 175,000",
    new: "New",
  },
  {
    Title: "McLaren Supercar",
    Image: require("../assets/18.png"),
    ratting: "4.9",
    disprice: "AED 115,000",
    price: "AED 175,000",
    new: "New",
  },
];

export const mywatchlist = [
  {
    Title: "Sedan Series",
    Image: require("../assets/watch1.png"),
    ratting: "4.9",
    disprice: "AED 115,000",
    price: "AED 175,000",
    new: "New",
  },
  {
    Title: "McLaren Supercar",
    Image: require("../assets/watch2.png"),
    ratting: "4.5",
    disprice: "AED 115,000",
    price: "AED 175,000",
    new: "Used",
  },
  {
    Title: "Bugatti Sports",
    Image: require("../assets/watch3.png"),
    ratting: "4.5",
    disprice: "AED 115,000",
    price: "AED 175,000",
    new: "Used",
  },
  {
    Title: "F-Type Jaguar",
    Image: require("../assets/Car4.png"),
    ratting: "3.5",
    disprice: "AED 115,000",
    price: "AED 175,000",
    new: "New",
  },
  {
    Title: "BMW Series",
    Image: require("../assets/Car5.png"),
    ratting: "4.3",
    disprice: "AED 115,000",
    price: "AED 175,000",
    new: "New",
  },
  {
    Title: "Classic Frog",
    Image: require("../assets/Car6.png"),
    ratting: "4.1",
    disprice: "AED 115,000",
    price: "AED 175,000",
    new: "New",
  },
];
export const OffersSpecial = [
  {
    Image: require("../assets/12.png"),
    percent: "20%",
    deals: "Week Deals!",
    dics: "Get a new car discount, only valid this week",
  },
  {
    Image: require("../assets/13.png"),
    percent: "10%",
    deals: "Top Deals!",
    dics: "Get a new car discount, only valid this week",
  },
  {
    Image: require("../assets/14.png"),
    percent: "15%",
    deals: "New Arrivals!",
    dics: "Get a new car discount, only valid this week",
  },
  {
    Image: require("../assets/15.png"),
    percent: "12%",
    deals: "Black Friday!",
    dics: "Get a new car discount, only valid this week",
  },
  {
    Image: require("../assets/16.png"),
    percent: "20%",
    deals: "Week Deals!",
    dics: "Get a new car discount, only valid this week",
  },
  {
    Image: require("../assets/17.png"),
    percent: "10%",
    deals: "Top Deals!",
    dics: "Get a new car discount, only valid this week",
  },
  {
    Image: require("../assets/18.png"),
    percent: "15%",
    deals: "New Arrivals!",
    dics: "Get a new car discount, only valid this week",
  },
  {
    Image: require("../assets/11.png"),
    percent: "12%",
    deals: "Black Friday!",
    dics: "Get a new car discount, only valid this week",
  },
];
export const toprated = [
  {
    Title: "BMW M4 Series",
    Image: require("../assets/13.png"),
    ratting: "4.5",
    disprice: "AED 115,000",
    price: "AED 175,000",
    new: "New",
  },
  {
    Title: "Camaro Sports",
    Image: require("../assets/14.png"),
    ratting: "4.7",
    disprice: "AED 175,000",
    price: "AED 175,000",
    new: "New",
  },
  {
    Title: "Audi Sports",
    Image: require("../assets/15.png"),
    ratting: "4.5",
    disprice: "AED 115,000",
    price: "AED 175,000",
    new: "New",
  },
  {
    Title: "McLaren Supercar",
    Image: require("../assets/16.png"),
    ratting: "4.9",
    disprice: "AED 115,000",
    price: "AED 175,000",
    new: "New",
  },
  {
    Title: "BMW M4 Series",
    Image: require("../assets/17.png"),
    ratting: "4.5",
    disprice: "AED 115,000",
    price: "AED 175,000",
    new: "New",
  },
  {
    Title: "Camaro Sports",
    Image: require("../assets/18.png"),
    ratting: "4.7",
    disprice: "AED 175,000",
    price: "AED 175,000",
    new: "New",
  },
  {
    Title: "Audi Sports",
    Image: require("../assets/13.png"),
    ratting: "4.5",
    disprice: "AED 115,000",
    price: "AED 175,000",
    new: "New",
  },
  {
    Title: "McLaren Supercar",
    Image: require("../assets/16.png"),
    ratting: "4.9",
    disprice: "AED 115,000",
    price: "AED 175,000",
    new: "New",
  },
];
export const Trusted = [
  {
    Title: "BMW M4 Series",
    Image: require("../assets/Image.png"),
    ratting: "4.5",
    disprice: "AED 115,000",
    price: "AED 175,000",
    new: "New",
  },
  {
    Title: "Camaro Sports",
    Image: require("../assets/Car2.png"),
    ratting: "4.7",
    disprice: "AED 175,000",
    price: "AED 175,000",
    new: "New",
  },
  {
    Title: "Audi Sports",
    Image: require("../assets/Car3.png"),
    ratting: "4.5",
    disprice: "AED 115,000",
    price: "AED 175,000",
    new: "New",
  },
];
export const cardetails = [
  {
    Image: require("../assets/Car2.png"),
    cartitle: "BMW M5 Series",
    NewText: "New",
    reviews: "4.8 (86 reviews)",
    description: "Description",
    distext:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore view more...",
    deposit: "Deposit",
    depositmoney: "AED 5000",
    kmtext: "KM Limit",
    kms: "3500km",
    insurance: "Insurance",
    insured: "3rd Party",
    transmission: "Transmission",
    included: "included",
    terms: "  Terms & Conditions: ",
    termsconditions: "Lorem ipsum dolor sit amet, consectetur adipiscing elit",
  },
];
export const cardetails2 = [
  {
    Image: require("../assets/Car.png"),
    cartitle: "BMW M5 Series",
    NewText: "New",
    reviews: "4.8 (86 reviews)",
    description: "Description",
    distext:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore view more...",
    deposit: "Deposit",
    depositmoney: "AED 5000",
    kmtext: "KM Limit",
    kms: "3500km",
    insurance: "Insurance",
    insured: "3rd Party",
    transmission: "Transmission",
    included: "included",
    terms: "  Terms & Conditions: ",
    termsconditions: "Lorem ipsum dolor sit amet, consectetur adipiscing elit",
  },
];
export const rent = [
  {
    authorize: "Authorized Dealer",
    monthly: "Monthly Rent",
    rental: "Monthly Rental",
    rentalprice: "AED 165,000",
  },
];
export const comments = [
  {
    img: require("../assets/avatar.png"),
    Name: "Darlene Robertson",
    numReview: "5",
    coment:
      "The car is awesome and very fast, my family and I really like it, wow!! 💯💯💯",

    likesnum: "729",
    days: "6 days ago",
  },
  {
    img: require("../assets/avatar.png"),
    Name: "Jenny Wilson",
    numReview: "5",
    coment:
      "The seat is very comfortable and shockproof, I will feel at home driving it. Highly recommended 👍👍",

    likesnum: "729",
    days: "6 days ago",
  },
  {
    img: require("../assets/avatar.png"),
    Name: "Marvin McKinney",
    numReview: "5",
    coment:
      "The car is very nice, fast and comfortable to drive. It's just that the price is not friendly 😂😂",

    likesnum: "729",
    days: "6 days ago",
  },
  {
    img: require("../assets/avatar.png"),
    Name: "Theresa Webb",
    numReview: "5",
    coment:
      "I just bought it, and it arrived at my house in just 4 days. Wow, I don't think there's anything faster 🔥🔥🔥",

    likesnum: "729",
    days: "6 days ago",
  },
  {
    img: require("../assets/avatar.png"),
    Name: "Darlene Robertson",
    numReview: "5",
    coment:
      "The car is awesome and very fast, my family and I really like it, wow!! 💯💯💯",

    likesnum: "729",
    days: "6 days ago",
  },
  {
    img: require("../assets/avatar.png"),
    Name: "Darlene Robertson",
    numReview: "5",
    coment:
      "The car is awesome and very fast, my family and I really like it, wow!! 💯💯💯",

    likesnum: "729",
    days: "6 days ago",
  },
];
export const Activebookings = [
  {
    Image: require("../assets/12.png"),
    Title: "BMW M5 Series",
    carcolor: "Silver",
    Delievery: "in Delievery",
    price: "$171,250",
    btntexts: "Track Order",
    backgroundColor: "#E7E7E7",
  },
  {
    Image: require("../assets/13.png"),
    Title: "Ferrari Sports",
    carcolor: "Yellow",
    Delievery: "in Delievery",
    price: "$186,500",
    btntexts: "Track Order",
    backgroundColor: "#FFC02D",
  },
  {
    Image: require("../assets/14.png"),
    Title: "Audi Sports",
    carcolor: "Red",
    Delievery: "in Delievery",
    price: "$134,250",
    btntexts: "Track Order",
    backgroundColor: "#F54336",
  },
  {
    Image: require("../assets/15.png"),
    Title: "McLaren Sports",
    carcolor: "Silver",
    Delievery: "in Delievery",
    price: "$156,500",
    btntexts: "Track Order",
    backgroundColor: "#E7E7E7",
  },
];
export const Completedbooking = [
  {
    img: require("../assets/16.png"),
    title: "BMW M5 Series",
    carcolor: "Silver",
    Delievery: "Completed",
    price: "$171,250",
    btntexts: "Leave Review",
  },
  {
    img: require("../assets/17.png"),
    title: "Ferrari Sports",
    carcolor: "Yellow",
    Delievery: "Completed",
    price: "$186,500",
    btntexts: "Leave Review",
  },
  {
    img: require("../assets/18.png"),
    title: "Audi Sports",
    carcolor: "Red",
    Delievery: "Completed",
    price: "$134,250",
    btntexts: "Leave Review",
  },
  {
    img: require("../assets/15.png"),
    title: "McLaren Sports",
    carcolor: "Silver",
    Delievery: "Completed",
    price: "$156,500",
    btntexts: "Leave Review",
  },
];
export const Track = [
  {
    img: require("../assets/Car.png"),
    title: "BMW M5 Series",
    price: "AED 35,000",
    rental: "Monthly Rental",
  },
];
export const chatdata = [
  {
    Image: require("../assets/Profile.png"),
    textname: "Zeeshan",
    textnum: "hi where are you ",
  },
  {
    Image: require("../assets/Profile.png"),
    textname: "Zeeshan",
    textnum: "hi where are you ",
  },
  {
    Image: require("../assets/Profile.png"),
    textname: "Zeeshan",
    textnum: "hi where are you ",
  },
  {
    Image: require("../assets/Profile.png"),
    textname: "Zeeshan",
    textnum: "hi where are you ",
  },
  {
    Image: require("../assets/Profile.png"),
    textname: "Zeeshan",
    textnum: "hi where are you ",
  },
  {
    Image: require("../assets/Profile.png"),
    textname: "Zeeshan",
    textnum: "hi where are you ",
  },
  {
    Image: require("../assets/Profile.png"),
    textname: "Zeeshan",
    textnum: "hi where are you ",
  },
  {
    Image: require("../assets/Profile.png"),
    textname: "Zeeshan",
    textnum: "hi where are you ",
  },
  {
    Image: require("../assets/Profile.png"),
    textname: "Zeeshan",
    textnum: "hi where are you ",
  },
  {
    Image: require("../assets/Profile.png"),
    textname: "Zeeshan",
    textnum: "hi where are you ",
  },
  {
    Image: require("../assets/Profile.png"),
    textname: "Zeeshan",
    textnum: "hi where are you ",
  },
];
export const invitation = [
  {
    Image: require("../assets/Profile.png"),
    textname: "Tysiana",
    textnum: "123456788",
  },
  {
    Image: require("../assets/Profile.png"),
    textname: "Tysiana",
    textnum: "123456788",
  },
  {
    Image: require("../assets/Profile.png"),
    textname: "Tysiana",
    textnum: "123456788",
  },
  {
    Image: require("../assets/Profile.png"),
    textname: "Tysiana",
    textnum: "123456788",
  },
  {
    Image: require("../assets/Profile.png"),
    textname: "Tysiana",
    textnum: "123456788",
  },
  {
    Image: require("../assets/Profile.png"),
    textname: "Tysiana",
    textnum: "123456788",
  },
  {
    Image: require("../assets/Profile.png"),
    textname: "Tysiana",
    textnum: "123456788",
  },
  {
    Image: require("../assets/Profile.png"),
    textname: "Tysiana",
    textnum: "123456788",
  },
  {
    Image: require("../assets/Profile.png"),
    textname: "Tysiana",
    textnum: "123456788",
  },
  {
    Image: require("../assets/Profile.png"),
    textname: "Tysiana",
    textnum: "123456788",
  },
  {
    Image: require("../assets/Profile.png"),
    textname: "Tysiana",
    textnum: "123456788",
  },
];
export const Mercedes = [
  {
    Title: "Mercedes",
  },
];
export const PartnerTrusted = [
  {
    Title: "Trusted Partner",
  },
];
export const Partnerr = [
  {
    Title: "Partner",
  },
];
export const OfferHeader = [
  {
    Title: "Sepcial Offers",
  },
];
export const Wishlist = [
  {
    Title: "My Wishlist",
  },
];
export const BookingHeader = [
  {
    Title: "My Bookings",
  },
];
export const Chat = [
  {
    Title: "Chat",
  },
];
export const BookingTrack = [
  {
    Title: "Track Bookings",
  },
];
export const EditProfileHeader = [
  {
    Title: "Edit Profile",
  },
];
export const AddressHeader = [
  {
    Title: "Address",
  },
];
export const mycars = [
  {
    Title: "My Cars",
  },
];
export const AddAddress = [
  {
    Title: "Add New Address",
  },
];
export const LanguageHeader = [
  {
    Title: "Language",
  },
];
export const PRivacyHEader = [
  {
    Title: "Privacy Policy",
  },
];
export const InviteHeader = [
  {
    Title: "Invite Friends",
  },
];
export const Chats = [
  {
    Title: "My Chats",
  },
];
export const HelpHeader = [
  {
    Title: "Help & Contact",
  },
];
export const ReviewHeader = [
  {
    Title: "4.8 (86 reviews)",
  },
];
export const DetailsHeader = [
  {
    Title: "Your Profile Details",
  },
];
export const ForgotPasswordHEader = [
  {
    Title: "Forgot Password",
  },
];
export const CreatePassHeader = [
  {
    Title: "Create New Password",
  },
];
export const AddressCar = [
  {
    Title: "Delievery Address",
  },
];
export const PopupActtive = [
  {
    img: require("../assets/Car.png"),
    title: "BMW M5 Series",
    carcolor: "Silver",
    Delievery: "in Delievery",
    price: "$171,250",
    btntexts: "Track Order",
    backgroundColor: "#E7E7E7",
  },
];
export const ScrollCars = [
  {
    img: require("../assets/CarView.png"),
  },
  {
    img: require("../assets/black.png"),
  },
  {
    img: require("../assets/red.png"),
  },
  {
    img: require("../assets/white.png"),
  },
  {
    img: require("../assets/white.png"),
  },
];
